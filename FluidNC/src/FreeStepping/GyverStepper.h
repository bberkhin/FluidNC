...
public:	
    // конструктор
    GStepper(int stepsPerRev, MotorDrivers::MotorDriver* drv) : _drv(drv)
    {
        // умолчания
        setMaxSpeed(300);
        setAcceleration(300);
    ...
    bool tick() {
        if (_workState) {
            ...
            if (_curMode && _accel != 0) 
                smoothSpeedPlanner();// планировщик скорости KEEP_SPEED
            ...
            if (stepTime && tickUs - _prevTime >= stepTime) {       // основной таймер степпера
                ...
                if (_pdir != dir) {
                   _pdir = dir;
                   _drv->set_direction((dir > 0));
                   config->_stepping->waitDirection();
                }
                _drv->step();  // двигаем мотор
    ...
    void setMaxSpeed(float speed) {
        speed = abs(speed);
        _maxSpeed = std::max(speed, _MIN_STEP_SPEED);	// 1 шаг в час минимум
...
private:
    ...
    float _accelInv = 0;
    int8_t _pdir       = 0;
    GS_runMode                 _curMode    = KEEP_SPEED;
    MotorDrivers::MotorDriver* _drv;
};
#endif
